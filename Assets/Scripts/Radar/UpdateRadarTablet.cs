...
public class UpdateRadarTablet : MonoBehaviour
{
    ...
    public RectTransform radarObjTransform;
    ...
    void Start()
    {
        ...
        // Set the sibling index of playerIcon to be higher than objectiveIcons
    ...
    //TODO fix the minimally wrong objetiveIcon position
    ...
    /*
    // It does not work and it makes me sad
    void UpdateIconRotation(RectTransform icon, Transform orientation)
    {
        // Check if the object reference is assigned
        if (orientation != null)
        {
            // Get the angle between the player's forward direction and the world forward direction
            float angle = Vector3.SignedAngle(Vector3.forward, orientation.forward, Vector3.up);
